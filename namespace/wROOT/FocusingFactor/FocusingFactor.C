...
void GetFocusingFactorAir(double zT, double xR, double zR, double focusing[2], double focusparts[6]){
...
void FocusingFactor(){
  ...
  // double GridStartX=50.5-(100/2);
  // double GridStopX=50.5+(100/2);
  ...
  for(int ix=0;ix<TotalStepsX_O;ix++){
    for(int iz=0;iz<TotalStepsZ_O;iz++){
      ...
      GetFocusingFactor(zT, xR, zR, focusing, focusparts);
      if(std::isnan(focusing[0])==false && focusing[0]!=1){
	gr2A->SetPoint(count, xR,zR, focusing[0]);
	...
	gr3A->SetPoint(count, xR,zR, focusing[1]);
	gr3B->SetPoint(count, xR,zR, focusparts[3]);
	gr3C->SetPoint(count, xR,zR, focusparts[4]);
	gr3D->SetPoint(count, xR,zR, focusparts[5]);
	count2++;
      }
