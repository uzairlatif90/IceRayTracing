...
void MakeRxTxPlots_Mult(){
  const char* filename;
  
  for(Int_t iTx=10;iTx<100;iTx=iTx+10){
    for(Int_t iTz=-1;iTz>-11;iTz--){
	...
	// for(Int_t idp=0;idp<NTxTot;idp++){
	//   for(Int_t iray=0;iray<3;iray++){
	//     delete gr[iray][idp];
	//     delete grB[iray][idp];
	//   }
	// }
